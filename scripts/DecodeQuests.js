...
import GetObjectiveData from '../modules/QuestObjectiveData.js';
...
import Objectives from '../input/QuestObjectives.json' assert {type: 'json'};
...
async function FetchAdditionalInfo(page, questId) {
...
for (const { id, nameId, repeatType, isPartyQuest, startCriterion, stepIds, categoryId } of Quests) {
   ...
   const { xp, kamas, mapId, coords } = await FetchAdditionalInfo(page, id);
